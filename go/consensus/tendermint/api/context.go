...
//
// Any existing references to the underlying State() should not be mutated while the checkpoint is
// open. Doing so may cause updates to leak to into the checkpoint as isolation is only one-way.
//
